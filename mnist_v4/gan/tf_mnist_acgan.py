...
if __name__ == "__main__":
    ...
    p.add_argument("--log_dir", type=str, default="log_tf_mnist_acgan")
    args = p.parse_args()
    ...
    D = acgan_mlp_discriminator() if args.mlp else acgan_cnn_discriminator()
    args.log_dir += "_mlp" if args.mlp else "_cnn"

    acgan = my.AuxCondGAN(G, D, noise_dim=args.noise_dim)
    acgan.compile(g_optim=keras.optimizers.Adam(learning_rate=1e-4),
    ...
    trained_D = acgan_mlp_discriminator() if args.mlp else acgan_cnn_discriminator()
