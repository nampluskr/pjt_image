...
class ACGanMlpDiscriminator(nn.Module):
    def __init__(self):
        ...
        self.fc_dis = nn.Linear(256, 1)
        self.fc_clf = nn.Linear(256, 10)
...
if __name__ == "__main__":
    ...
    p.add_argument("--log_dir", type=str, default="log_pt_mnist_acgan")
    args = p.parse_args()
    ...
    acgan.compile(g_optim=torch.optim.Adam(G.parameters(), lr=1e-4),
    ...
    hist = my.train(acgan, train_loader, args, sample_noises)
    ...
    if args.mlp:
        ...
        trained_D = ACGanMlpDiscriminator().to(device)
    else:
        trained_G = CGanCnnGenerator().to(device)
        trained_D = ACGanCnnDiscriminator().to(device)
