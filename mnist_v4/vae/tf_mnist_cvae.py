...
def get_mlp_encoder(latent_dim=2):
    ...
    return keras.Model(inputs=[images, labels], outputs=[z_mean, z_log_var])
...
if __name__ == "__main__":
    ...
    p.add_argument("--log_dir", type=str, default="log_tf_mnist_cvae")
    args = p.parse_args()
    ...
    cvae.compile(optim=keras.optimizers.Adam(),
    ...
    trained_cvae.compile(optim=keras.optimizers.Adam(),
               loss_fn_bce=loss_fn_bce,
               loss_fn_kld=loss_fn_kld,
               metric_fn=binary_accuracy, metric_name="acc")
