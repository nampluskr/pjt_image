...
def get_mlp_encoder(latent_dim=2):
    ...
    return keras.Model(inputs=[images, labels], outputs=outputs, name="encoder")


def get_mlp_decoder(latent_dim=2):
    noises = keras.Input(shape=(latent_dim,))
    labels = keras.Input(shape=(1,))
    labels_ = tf.reshape(tf.one_hot(tf.cast(labels, tf.int64), depth=10), shape=(-1, 10))
    ...
    return keras.Model(inputs=[noises, labels], outputs=outputs, name="decoder")


def get_cnn_encoder(latent_dim=2, n_classes=10, embedding_dim=100):
    images = keras.Input(shape=(28, 28, 1))
    labels = keras.Input(shape=(1,))
    labels_ = layers.Embedding(n_classes, embedding_dim)(labels)
    labels_ = layers.Dense(28*28*1)(labels_)
    labels_ = layers.Reshape((28, 28, 1))(labels_)

    x = layers.Concatenate()([images, labels_])
...
def get_cnn_decoder(latent_dim=2, n_classes=10, embedding_dim=100):
    noises = keras.Input(shape=(latent_dim,))
    noises_ = layers.Dense(256)(noises)
    noises_ = layers.ReLU()(noises_)
    noises_ = layers.Dense(7*7*64)(noises_)
    noises_ = layers.ReLU()(noises_)
    noises_ = layers.Reshape((7, 7, 64))(noises_)

    labels = keras.Input(shape=(1,))
    labels_ = layers.Embedding(n_classes, embedding_dim)(labels)
    labels_ = layers.Dense(7*7*1)(labels_)
    labels_ = layers.Reshape((7, 7, 1))(labels_)

    x = layers.Concatenate()([noises_, labels_])
...
if __name__ == "__main__":
    ...
    p.add_argument("--log_dir", type=str, default="log_tf_mnist_cae")
    args = p.parse_args()
    ...
    cae = my.CondAutoEncoder(encoder, decoder)
    cae.compile(optim=keras.optimizers.Adam(),
               loss_fn=keras.losses.MeanSquaredError(),
               metric_fn=binary_accuracy, metric_name="acc")

    hist = my.train_with_metric(cae, train_loader, valid_loader, args)
    ...
    trained_cae = my.CondAutoEncoder(trained_encoder, trained_decoder)
    trained_cae.compile(optim=keras.optimizers.Adam(),
               loss_fn=keras.losses.MeanSquaredError(),
               metric_fn=binary_accuracy, metric_name="acc")

    loss, acc = my.evaluate(trained_cae, valid_loader)
