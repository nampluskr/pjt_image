...
class TrainerWithMetric(nn.Module):
    ...
    def train_step(self, data):
        ...
        metric = self.metric_fn(y_pred, y)
...
def train_with_metric(trainer, train_loader, valid_loader, args):
    ...
    for epoch in range(args.n_epochs):
        ...
        with tqdm(train_loader, total=len(train_loader), ncols=100,
            ...
            for x, y in pbar:
                data = x.to(trainer.device), y.to(trainer.device)
        ...
        if val_loss.mean() < best_loss and (best_loss - val_loss.mean()) > args.min_loss:
            ...
            best_epoch = epoch + 1
            best_model = deepcopy(trainer.model.state_dict())
            counter = 1
        else:
            counter += 1
...
def train(trainer, train_loader, args):
    ...
    torch.save(trainer.model, os.path.join(log_path, args.log_dir + "_weights.pth"))
    return hist
...
def evaluate(trainer, test_loader):
    ...
    with tqdm(test_loader, total=len(test_loader), ncols=100,
        ...
        for x, y in pbar:
            ...
            loss = np.append(loss, results['loss'].item())
            metric = np.append(metric, results[trainer.metric_name].item())
            pbar.set_postfix({'loss': "%.4f" % loss.mean(),
